...
# two flags: -s (shift Ti), -o (output iterdb file)
parser = op.OptionParser()
parser.add_option('--shift','-s',action='store_const',const=1,help='shift Ti and Er profile out by 0.005',default=0)
parser.add_option('--output_iterdb','-o',action='store_const',const=1,help='write out iterdb file from p file',default=0)
options,args = parser.parse_args()
# 4 file names as input for reading iterdb file and compare with p-/Er-file
# 3 file names as input for writing iterdb file
if len(args) == 4:
...
# Er file use kV/m as unit of Er, here convert into V/m
Er = Er*1E03

# psi0_Er starts from 0.905 to 1.055, psip_n_obmp goes from 0 to 1.06 
# range of psip_n_obmp is broader than the range of psi0_Er
...
print psi_Er_l-psi_Er_f

psipn_Er = np.linspace(psip_n_obmp[psi_Er_f],psip_n_obmp[psi_Er_l],500)
Er_obmp = interp(psi0_Er,Er,psipn_Er)
rhotn_Er = rho_tor_spl(psipn_Er*(psisep-psiax)+psiax)

### output Er vs psip_n
#plt.plot(psipn_Er,Er_obmp,label='obmp')
#plt.plot(psi0_Er,Er,label='Er file')
#plt.legend()
#plt.show()
...
#plt.plot(rhot_gene, omega_tor_gene,label='omega_gene')
#plt.plot(rhot0,omega_tor_full,label='omega0')
#plt.legend()
#plt.show()
