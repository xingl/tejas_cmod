...
if show_plots:
    #plt.plot(psi/(psisep-psiax),qpsi,'x')
...
#plt.show()
    ...
    #plt.contour(Rgrid,Zgrid,(psirz[:]-psiax)/(psisep-psiax),150)
    #plt.colorbar()
    #plt.contour(Rgrid,Zgrid,(psirz[:]-psiax)/(psisep-psiax),levels = [0.999],color = 'black',linewidth = 2)
...
#plt.contour(Rgrid,Zgrid,(psirz[:]-psiax)/(psisep-psiax),levels = [1.0],color = 'black', linewidth = 2)
    #plt.contour(Rgrid,Zgrid,(psirz[:]-psiax)/(psisep-psiax),levels = [0.9,0.95,1.0],color = 'black', linewidth = 4)
...
if write_binfo:
    ...
    f.write('# 1.R(m) 2.psi_norm 3.B_pol(T) 4.B_tor(T) 5.q 6.rho_tor\n')
    ...
    B_tor = F_out / Rgrid
    np.savetxt(f,np.column_stack((Rgrid[Rmag_ind:psi_ind_sep],psi_norm_out[Rmag_ind:psi_ind_sep],B_pol[Rmag_ind:psi_ind_sep],B_tor[Rmag_ind:psi_ind_sep],q_out[Rmag_ind:psi_ind_sep],rho_tor_out[Rmag_ind:psi_ind_sep])))
    f.close()
...
#print rbs_string
#rbs = np.zeros(len(rbs_string))
rbc = np.empty(0)
zbs = np.empty(0)
#zbc = np.zeros(len(rbs_string))
...
#num = np.arange(len(rbc),dtype='int')
...
num = np.arange(len(rbc),dtype='int')
f=open('rbc_zbs_njet.temp','w')
f.write('#'+ivfile_name+'\n')
f.write('# num rbc zbs'+'\n')
np.savetxt(f,np.column_stack((num,rbc,zbs)))
f.close()
...
for i in range(2000):
    ...
    for j in range(len(num)):
        ...
        this_r = this_r + rbc[j]*math.cos(arg)
        this_z = this_z + zbs[j]*math.sin(arg)
...
plt.title(efile_name +'&'+  ivfile_name)
#plt.xlabel('r')
#plt.ylabel('z')
plt.show()
